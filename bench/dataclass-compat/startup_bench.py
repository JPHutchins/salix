...
results.append(bench(HYDRA_VENV, "hydra stock     (example config)", "import sys; sys.path.insert(0, '/home/jp/repos/hydra'); " + hydra_core, cwd="/home/jp/repos/hydra/tests"))
results.append(bench(HYDRA_VENV, "hydra patched   (example config)", "import sys; sys.path.insert(0, '/home/jp/repos/hydra'); sys.path.insert(0, '" + SHIM + "'); from _shim import install; install(exclude_prefixes=('hydra',)); " + hydra_core, cwd="/home/jp/repos/hydra/tests"))

results.append(bench(TYRO_VENV, "tyro stock   (cli parse)", "import runpy; runpy.run_path('/tmp/tyro_app.py')"))
results.append(bench(TYRO_VENV, "tyro patched (cli parse)", "import sys; sys.path.insert(0, '" + SHIM + "'); from _shim import install; install(exclude_prefixes=('tyro',)); import runpy; runpy.run_path('/tmp/tyro_app.py')"))
